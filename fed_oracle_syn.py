...
def main():
    ...
    sythetic_path = open(args.syn_path, "r").readlines()
    syn_images = []
    syn_labels = []
    for line in sythetic_path:
        image_path, label_path = line.strip().split(" ")
        syn_images.append(torch.load(image_path.strip(), map_location="cpu").requires_grad_(False))
        syn_labels.append(torch.load(label_path.strip(), map_location="cpu").requires_grad_(False))
    ...
    trainer = SerialTrainer(model=local_model,
                                ...
                                cuda=args.cuda,
                                logger=logger,
                                sythetic_image=image_syn,
                                sythetic_label=label_syn,
                                args={
                                    "batch_size": 64,
                                    "epochs": 1,
                                    "max_norm": 10,
